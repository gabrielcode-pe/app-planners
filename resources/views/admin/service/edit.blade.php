@extends('layouts.app')
@section('titulo') Editar servicio de {{$service->title}} @endsection
...
<div class="row mb-2">
    ...
    <form action="{{ url('/panel/service/'.$service->id) }}" method="post" enctype="multipart/form-data">
    {{ csrf_field() }}
    {{ method_field('PUT')}}
    ...
    <div class="row">
        ...
        <section class="col-12 col-md-10">
            <div class="form-group">
                <input type="text" class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" id="title" name="title" aria-describedby="title" placeholder="Ingrese el servicio" value="{{$service->title}}" required>
                @if ($errors->has('title'))
        ...
        <section class="col-12 col-md-8">
            <div class="form-group">
                <textarea name="info" id="info" class="form-control ckeditor {{ $errors->has('info') ? ' is-invalid' : '' }}" rows="5" placeholder="Ingrese la información del servicio">{{$service->info}}</textarea>                
                @if ($errors->has('info'))
                <div id="validationServer03Feedback" class="invalid-feedback">
                    {{ $errors->first('info') }}
                </div>
